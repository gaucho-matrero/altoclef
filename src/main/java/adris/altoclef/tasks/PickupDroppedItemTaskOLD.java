...
@Deprecated
public class PickupDroppedItemTaskOLD extends Task {
    ...
    public PickupDroppedItemTaskOLD(List<ItemTarget> itemTargets) {
        _itemTargets = itemTargets;
        _itemGoal = null;
        _progressChecker = new LinearProgressChecker(7.0, 0.1);
    }

    public PickupDroppedItemTaskOLD(Item item, int targetCount) {
...
//        setDebugState("FOUND: " + (closest.getStack() != null? closest.getStack().getItem().getTranslationKey() : " (nothing)"));
    ...
    protected void onStop(AltoClef mod, Task interruptTask) {
        ...
        if (!(interruptTask instanceof PickupDroppedItemTaskOLD)) {
            mod.getClientBaritone().getCustomGoalProcess().onLostControl();
    ...
    protected boolean isEqual(Task other) {
        // Same target items
        if (other instanceof PickupDroppedItemTaskOLD) {
            PickupDroppedItemTaskOLD t = (PickupDroppedItemTaskOLD) other;
