...
public class BeatMinecraftTask extends Task {
    ...
    private Task overworldTick(AltoClef mod) {
        ...
        if (!isEndPortalOpened(mod)) {
            ...
            if (!hasDiamondArmor(mod) || !mod.getInventoryTracker().hasItem(Items.DIAMOND_PICKAXE) || !mod.getInventoryTracker().hasItem(Items.DIAMOND_SWORD) || (needsToGoToNether && mod.getInventoryTracker().getItemCountIncludingTable(ItemHelper.LOG) <= 0)) {
    ...
    private Task netherTick(AltoClef mod) {
        ...
        int planksCount = 4 * mod.getInventoryTracker().getItemCount(ItemHelper.LOG) + mod.getInventoryTracker().getItemCount(ItemHelper.PLANKS);
