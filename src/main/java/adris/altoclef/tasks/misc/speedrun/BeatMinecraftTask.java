...
public class BeatMinecraftTask extends Task {
    ...
    private final Task _prepareForDiamondCollectionTask = TaskCatalogue.getSquashedItemTask(new ItemTarget("iron_pickaxe", 3));
    private final Task _netherPrepareTaskJustPick = TaskCatalogue.getItemTask("wooden_pickaxe", 1);
    private final Task _netherPrepareTaskWood = TaskCatalogue.getSquashedItemTask(new ItemTarget("wooden_pickaxe", 1),
    ...
    private Task overworldTick(AltoClef mod) {
        ...
        if (!strongholdPortalFound()) {
            if (mod.getInventoryTracker().getItemCountIncludingTable(Items.ENDER_EYE) > 1 &&
                (_strongholdLocater.isActive() || _strongholdLocater.isSearching()) && !_strongholdLocater.isFinished(mod)) {
                ...
                if (_netherPortalPos != null) {
                    if (mod.getBlockTracker().isTracking(Blocks.NETHER_PORTAL)) {
                        if (!mod.getBlockTracker().blockIsValid(_netherPortalPos, Blocks.NETHER_PORTAL)) {
                            ...
                            if (mod.getChunkTracker().isChunkLoaded(_netherPortalPos) || _netherPortalPos.getSquaredDistance(
                                    mod.getPlayer().getPos(), false) > MAX_PORTAL_DISTANCE * MAX_PORTAL_DISTANCE) {
                                Debug.logMessage(
                                        "Invalid portal position detected at " + _netherPortalPos + ", finding new nether portal.");
    ...
    private Task foundPortalGetToEndTask(AltoClef mod) {
        ...
        if (_endBedSpawnPos == null) {
            ...
            if (mod.getInventoryTracker().getItemCount(Items.DIRT, Items.COBBLESTONE, Items.NETHERRACK) < MINIMUM_BUILDING_BLOCKS &&
                _collectBuildMaterialsTask.isActive() && !_collectBuildMaterialsTask.isFinished(mod)) {
