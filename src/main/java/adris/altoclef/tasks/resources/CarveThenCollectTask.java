...
public class CarveThenCollectTask extends ResourceTask {
    ...
    protected Task onResourceTick(AltoClef mod) {
        ...
        if (mod.getBlockTracker().anyFound(_targetBlocks)) {
            ...
            return new DoToClosestBlockTask(() -> mod.getPlayer().getPos(), DestroyBlockTask::new,
                                            pos -> mod.getBlockTracker().getNearestTracking(pos, _targetBlocks), _targetBlocks);
        }
        ...
        if (mod.getBlockTracker().anyFound(_toCarveBlocks)) {
            ...
            return new DoToClosestBlockTask(() -> mod.getPlayer().getPos(),
                                            blockPos -> new InteractItemWithBlockTask(_carveWith, blockPos, false),
                                            pos -> mod.getBlockTracker().getNearestTracking(pos, _toCarveBlocks), _toCarveBlocks);
        }
    ...
    protected boolean isEqualResource(ResourceTask obj) {
        if (obj instanceof CarveThenCollectTask) {
            ...
            return (task._target.equals(_target) && task._toCarve.equals(_toCarve) && Util.arraysEqual(task._targetBlocks, _targetBlocks) &&
                    Util.arraysEqual(task._toCarveBlocks, _toCarveBlocks));
        }
        return false;
    }
